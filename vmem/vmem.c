...
#define	OCCP_TABLE_GET_ADDR(n)	(OCCP_TABLE_ADDR + PAGE_SIZE*n)
...
/*uint8_t* get_phy_space( unsigned int nbFrames )
{
	int i;
	int j;
	for( i = 0; i < OCCP_TABLE_COUN; ++i)
	{
		for(j = i; j < i+nbFrames; ++j)
		{
			if(occupation_table[j] != 0)
			{
				i = j+1;
				break;
			}
